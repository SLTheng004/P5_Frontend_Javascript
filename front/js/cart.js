...
const orderId = urlParameter.get('orderId');
  ...
  // defining the LoadCart Function
const loadCart = () => {
  ...
  let cart_quantity = 0;
  let cart_price = 0;
    //Loop through product array fom local storage
...
const deleteItem = (id, color) => {
  ...
  // get local storage array
  let products = JSON.parse(localStorage.getItem("cart"));
  // target id and color of item to be deleted
...
//validate an input, and send error message if requirements are not met
const validateInput = (Input) =>{
  ...
  if (/[^a-zA-Z]/.test(nameValue)){
    firstNameErrorMsg.innerHTML =`Please provide a first name that contains only letters.`;
    output= false;
  }
  ...
  if (/[^a-zA-Z]/.test(lastNameValue)){
    lastNameErrorMsg.innerHTML =`Please provide a last name that contains only letters.`;
    output= false;
  }
  // validate empty value
  if(value=="" ){
    erroMsg.innerHTML =`Please provide the ${Input}.`;
    output= false;
  }
  // validate Length
  if(value.length <3){
